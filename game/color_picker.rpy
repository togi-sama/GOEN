...
init python:
    ...
    class ColorPicker(renpy.Displayable):
        ...
        def __init__(self, xsize, ysize, start_color=None, four_corners=None,
            ...
            ## Convert any saved colours to Color objects
            for k, v in self.saved_colors.items():
                self.saved_colors[k] = Color(v)
        ...
        def swatch(self, key=None, xsize=100, ysize=100,
                refresh_rate=1.0/60.0):
            """
            Returns a DynamicDisplayable for the saved color at the given key.
            If it is not the currently active colour, it simply returns the
            color without any dynamic updates.
            """
            if key is not None and key != self.last_saved_color:
                refresh_rate = None
            return DynamicDisplayable(picker_color, picker=self, key=key,
                xsize=xsize, ysize=ysize, refresh_rate=refresh_rate)

        def hexcode(self, key=None, style='picker_hexcode',
                refresh_rate=1.0/60.0):
            """
            Returns a DynamicDisplayable for the hex code of the saved color
            at the given key. If it is not the currently active colour, it
            simply returns the hex code without any dynamic updates.
            """
            if key is not None and key != self.last_saved_color:
                refresh_rate = None
            return DynamicDisplayable(picker_hexcode, picker=self, key=key,
                style=style, refresh_rate=refresh_rate)

        def SetColor(self, color, key=None):
            """
            A screen action which will set the key of the picker to the
            provided one and update its color to the provided color. If no key
            is provided, it sets the currently active key to the provided color.
            """
            if key is None:
                key = self.last_saved_color
            return [SelectedIf(Color(self.get_color(key)) == Color(color)),
                SetField(self, 'last_saved_color', key),
                Function(self.set_color, color, _update_screens=False),
                Function(self.save_color, key, _update_screens=False),
                Function(renpy.redraw, self, 0)]

        def SwapToSavedColor(self, key):
            """
            A screen action which will save the current color and swap to the
            saved color at the provided key.
            """
            return [SensitiveIf(key != self.last_saved_color),
                SelectedIf(self.last_saved_color==key),
                Function(self.save_color, self.last_saved_color, _update_screens=False),
                Function(self.swap_to_saved_color, key)]
    ...
    def picker_color(st, at, picker, xsize=100, ysize=100, key=None,
            refresh_rate=1.0/60.0):
        """
        ...
        """
        if key is None:
            return Transform(picker.color, xysize=(xsize, ysize)), refresh_rate
        return Transform(picker.get_color(key), xysize=(xsize, ysize)), refresh_rate

    def picker_hexcode(st, at, picker, style='picker_hexcode', key=None,
            refresh_rate=1.0/60.0):
        """
        ...
        """
        if key is None:
            return Text(picker.color.hexcode, style=style), refresh_rate
        return Text(picker.get_color(key).hexcode, style=style), refresh_rate
